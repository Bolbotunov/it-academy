function calculator(str) {
    ...
    str.forEach((item, index) => {
        if (item in brackets) {
            ...
            while (operatorsStack.length && operations[item] <= operations[operatorsStack[operatorsStack.length - 1]]) {
                completeStack.push(operatorsStack.pop());
            }
            operatorsStack.push(item);
        }
  ...
  function test() {
    ...
    console.log(`тест 2*-(2+2): Ожидаемый ответ: -8, Полученный ответ: ${calculator('2*-(2+2)')}`)
    console.log(`тест -2*-(2+2): Ожидаемый ответ: 8, Полученный ответ: ${calculator('-2*-(2+2)')}`)
    console.log(`тест -5*4-(-10+5*(4-5*-1))+5: Ожидаемый ответ: -50, Полученный ответ: ${calculator('-5*4-(-10+5*(4-5*-1))+5')}`)
  }
  test()
